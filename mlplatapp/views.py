    ...
    if request.method == 'GET':
        ...
        for data_item in data_info:
            if models.Data.objects.filter(data_name=data_item):
                ...
                models.DropColl('stat_data_quality_' + data_item, host=HOST, port=PORT, database=DATABASE)  # 质量检测过程的中间结果，使用过就没用了，删除
...
def upload(request):
    if request.method == 'POST':
        ...
        stat_data_quality_name, res = models.SavaData('stat_data_quality_' + data_name_in_db, stat_data_quality,
                                                      host=HOST, port=PORT, database=DATABASE)

        algo_data_quality = []  # 检测表格数据质量，得到算法检测结果
        for key, val in excelproc.algorithm_data_check().items():
            algo_data_quality.append({key: val})
        algo_data_quality_name, res = models.SavaData('algo_data_quality_' + data_name_in_db, algo_data_quality,
                                                      host=HOST, port=PORT, database=DATABASE)
...
def qualitycontrol(request, data_name, stat_quality_name, algo_quality_name):
    if request.method == 'POST':
        pass

    if request.method == 'GET':
        data = models.ReadData(data_name,host=HOST, port=PORT, database=DATABASE)
        stat = models.ReadData(stat_quality_name,host=HOST, port=PORT, database=DATABASE)
        algo = models.ReadData(algo_quality_name,host=HOST, port=PORT, database=DATABASE)
...
def download(request, name):
    if request.method == 'GET':
        ...
        WFP = os.path.join(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'excel_file'), str(name))
